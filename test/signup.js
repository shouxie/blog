	...
	describe('POST / signup', function() {
		...
		it('wrong bio', function(done) {
			agent
				.post('/signup')
				.type('form')
				.field({name:testName2, gender: 'm', bio: '' })
				.attach('avatar', path.join(__dirname, 'avatar.jpg'))
				.redirects()
				.end(function(err, res) {
					if (err) return done(err);
		...
		it('duplicate name', function(done) {
			agent
				.post('/signup')
				.type('form')
				.field({name: testName1, gender: 'm', bio: 'noder', password: '111111', repassword: '111111'})
		...
		it('success', function(done) {
			agent
				.post('/signup')
				.type('form')
				.field({ name: testName2, gender: 'm', bio: 'noder', passward: '111111', repassword: '111111'})
				.attach('avatar', path.join(__dirname, 'avatar.jpg'))
				.redirects()
				.end(function(err, res) {
					if (err) return done(err);
